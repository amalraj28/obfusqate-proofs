...
lemma delayedGates:
  assumes pos_lt: "pos < length qc"
  assumes n_lt: "n < length (delayed_seq (qc ! pos))"
  assumes qc_carrier: "\<forall>g \<in> set qc. g \<in> carrier_mat d d"
  assumes seq_carrier: "\<forall>g \<in> set ((delayed_seq (qc ! pos)) ! n). g \<in> carrier_mat d d"
  shows "compose (replace_gate qc pos ((delayed_seq (qc ! pos)) ! n)) d = compose qc d"
...
proof -
  ...
  have seq_eq_dimrow:
    "compose ((delayed_seq (qc ! pos)) ! n) (dim_row (qc ! pos)) = qc ! pos"
    using delayed_seq_correct_idx[OF n_lt]
    by simp

  have seq_eq:
    "compose ((delayed_seq (qc ! pos)) ! n) d = qc ! pos"
